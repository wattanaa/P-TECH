...
import { 
  Search, BookOpen, GraduationCap, Clock, CheckCircle, 
  ArrowRight, ShieldCheck, HelpCircle, Briefcase, Sparkles, X,
  Settings, Zap, Cpu, TrendingUp
} from "lucide-react";
...
export default function CurriculumView({ 
  ...
  const [selectedLevelFilter, setSelectedLevelFilter] = useState<"ทั้งหมด" | "ปวช." | "ปวส.">("ทั้งหมด");

  const [searchQuery, setSearchQuery] = useState("");
  const [activeDetailsMajor, setActiveDetailsMajor] = useState<Major | null>(null);

  // Filter majors based on selected Level and Search query
  const filteredMajors = useMemo(() => {
    return majors.filter((major) => {
      const matchesLevel = selectedLevelFilter === "ทั้งหมด" || major.level === selectedLevelFilter;
      const matchesSearch = 
        major.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        major.englishName.toLowerCase().includes(searchQuery.toLowerCase()) ||
        major.description.toLowerCase().includes(searchQuery.toLowerCase()) ||
        major.features.some(f => f.toLowerCase().includes(searchQuery.toLowerCase())) ||
        major.careerPaths.some(c => c.toLowerCase().includes(searchQuery.toLowerCase()));
      return matchesLevel && matchesSearch;
    });
  }, [selectedLevelFilter, searchQuery]);

  const handleApplyClick = (major: Major) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10" id="curriculum-view">
      {/* Header section */}
      ...
      <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-2xl border border-slate-200 gap-4 shadow-sm" id="filters-row">
        ...
        <div className="flex space-x-1.5 bg-slate-100 p-1 rounded-xl w-full md:w-auto">
          {(["ทั้งหมด", "ปวช.", "ปวส."] as const).map((level) => (
            <button
              key={level}
              onClick={() => setSelectedLevelFilter(level)}
              className={`flex-1 md:flex-none px-5 py-2.5 rounded-lg text-sm font-semibold transition-all duration-150 ${
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2 pt-2">
                  ...
                  <ul className="space-y-1.5 text-xs text-slate-600">
                    {major.features.slice(0, 2).map((feature, i) => (
                      <li key={i} className="flex items-center space-x-1.5 line-clamp-1">
              ...
              <div className="p-6 md:p-8 space-y-6 text-slate-700">
                ...
                <div className="space-y-3">
                  ...
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-2.5 text-xs">
                    {activeDetailsMajor.features.map((feature, i) => (
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs">
                    {activeDetailsMajor.careerPaths.map((career, i) => (
                      <span 
                        key={i} 
