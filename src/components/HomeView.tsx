...
import { 
  ArrowRight, BookOpen, GraduationCap, Users, Award, 
  Settings, Zap, Cpu, TrendingUp, Calendar, Eye, ShieldCheck, 
  Flame, HelpCircle, ChevronRight, Database, DownloadCloud, 
  HeartHandshake
} from "lucide-react";
...
export default function HomeView({ setActiveTab, setSelectedNews }: HomeViewProps) {
  ...
  const statCards = [
    { value: `${new Date().getFullYear() + 543 - parseInt(collegeInfo.foundedYear)} ปี`, label: "แห่งการก่อตั้งและดูแลนักเรียน", description: "เติบโตอย่างมั่นคงเคียงคู่ท้องถิ่น", icon: Award, color: "text-cyan-500" },
          ...
          {heroSlides.map((_, idx) => (
            <button
              ...
              className={`w-3 h-3 rounded-full transition-colors duration-150 ${
                currentSlide === idx ? "bg-brand-primary" : "bg-white/40"
              }`}
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-brand-primary to-blue-800 flex items-center justify-center text-white">
                      {portal.icon === "Database" ? <Database className="w-5 h-5 text-amber-300" /> :
                       portal.icon === "Award" ? <Award className="w-5 h-5 text-amber-300" /> :
                       portal.icon === "GraduationCap" ? <GraduationCap className="w-5 h-5 text-amber-300" /> :
                       portal.icon === "Calendar" ? <Calendar className="w-5 h-5 text-amber-300" /> :
                       portal.icon === "DownloadCloud" ? <DownloadCloud className="w-5 h-5 text-amber-300" /> :
            ...
            {[
              ...
              >
                <div className={`p-2 rounded-lg bg-slate-50 group-hover:bg-blue-50 transition-colors ${partner.color}`}>
                  {partner.icon === "Building" ? <Users className="w-5 h-5" /> :
                   partner.icon === "Settings" ? <Settings className="w-5 h-5" /> :
                   partner.icon === "Award" ? <Award className="w-5 h-5" /> :
                   partner.icon === "Cpu" ? <Cpu className="w-5 h-5" /> :
                   partner.icon === "Zap" ? <Zap className="w-5 h-5" /> :
